...
public class TreasureHunt extends PApplet {
    ...
    public static void main(String[] args){
        ...
        // do not add any other statement to your TreasureHunt.main()
    }
    /**
     ...
     private void loadGameSettings(String filename) {
         ...
         try {
             ...
             backgroundImageFilename = "images" + File.separator + backgroundImageFilename
                     + ".png"; backgroundImage = loadImage(backgroundImageFilename);
             lineNumber++;
    ...
    private InteractiveObject findObjectByName(String name) {
        ...
        System.out.println("WARNING: Failed to find an interactive object with name: " + name); return null;
    }

    /**
     * This method creates and returns a new ClickableObject based on the properties * specified as strings within the provided parts array.
     *
     ...
     * - name of object to activate (optional): activates this object when clicked * @return the newly created object
     */
    private ClickableObject loadNewClickableObject(String[] parts) { // C: name: x: y: message: name of object to activate (optional) // parse parts:
    ...
    private DroppableObject loadNewDroppableObject(String[] parts) {
        ...
        if (!(dropTarget instanceof VisibleObject))
            dropTarget = null;
        String message = parts[5].trim(); InteractiveObject activate = null;
        if (parts.length > 6)
    ...
    @Override
    public void setup(){
        gameObjects = new ArrayList<InteractiveObject>(); // initialize it to an empty ArrayList object
    ...
    public void draw(){
        ...
        for (int i=0; i<gameObjects.size(); i++){
            if (gameObjects.get(i).isActive()) {
                Action atn = gameObjects.get(i).update();
                if (atn != null) {
                    atn.act(gameObjects);
                }
            }
